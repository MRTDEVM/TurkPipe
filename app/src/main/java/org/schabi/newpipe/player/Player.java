...
public final class Player implements PlaybackListener, Listener {
    ...
    public void triggerProgressUpdate() {
        ...
        if (currentState == STATE_PLAYING) {
            final long currentPos = simpleExoPlayer.getCurrentPosition();
            final org.schabi.newpipe.player.helper.SponsorBlockHelper.Segment skipSegment =
                    org.schabi.newpipe.player.helper.SponsorBlockHelper.checkSkip(currentPos);
            if (skipSegment != null) {
                final long skipToMs = (long) (skipSegment.end * 1000);
                if (DEBUG) {
                    Log.d(TAG, "SponsorBlock - skipping " + skipSegment.category
                            + " to " + skipToMs + "ms");
                }
                seekTo(skipToMs);
                android.widget.Toast.makeText(context,
                        "Sponsor segment skipped (" + skipSegment.category + ")",
    ...
    private void updateMetadataWith(@NonNull final StreamInfo info) {
        ...
        if (info.getServiceId() == 0) { // YouTube service ID
            org.schabi.newpipe.player.helper.SponsorBlockHelper.fetchSegments(info.getId());
        } else {
            org.schabi.newpipe.player.helper.SponsorBlockHelper.fetchSegments(null);
        }
